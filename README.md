...
- [1) Preqrequisites](#1-prerequisites)
...
- [7) Project structure (quick tour)](#7-project0structure-quick-tour)
...
Start an API that implements the `/accounts/*` endpoints described in the [How authentication works](#5-how-authentication-works)
...
This runs `ng serve --open` and should open the appp in your browser.
...
3. If you are using the fake backend, a "verification email" will appear as an alaert with a link
4. Click the verification link (or paste it in the browser) to verify your account

### B) Login

1. Go to Login
2. Enter yoru email + password
...
- Acces token (JWT): short-lived token used in the `Authorization: Bearer <token>` header
- Refresh token: long-lived stored in a cokkie and sent with `withCredentials: true`
...
Role restrictions are applied using route data, for examle:

- `/admin` requireds `Role.Admin`
...
- `_helpers/` corss-cutting helpers (guards, interceptors, app initializer, fake backend)
...
The UI is styled with Boostrap 5 via a CON link in:
...
- If your API runs on a different origin (different hostname/port), you must configure CORS to allow credentials and ensure cookes are set with the correct `SameSite`/`Secure` settings.
